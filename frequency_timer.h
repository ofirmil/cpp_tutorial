...
template <int Hz>
using hz_to_duration =
    typename std::chrono::duration<int64_t, std::ratio<1, Hz> >;

template <int Hz>
class frequency_timer_t {
 public:
  frequency_timer_t(std::chrono::steady_clock::time_point initial_wakeup_time =
  ...
  void wait() {
    ...
    if (current_time < wakeup_time_) {
      ...
      std::cerr << "warning: frequency_timer_t::wait() - calling thread passed "
                   "wakeup time"
                << std::endl
                << std::flush;
