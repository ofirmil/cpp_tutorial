...
brain_t::brain_t(const string &yaml_config)
    : config_{yaml_config},
      motors_positions_products_{
          motors_positions_hz * 2 /
          main_loop_hz},  // double the expected size for every round
      robot_positions_products_{robot_positions_hz * 2 / main_loop_hz}
// double the expected size for every round
{}
  ...
  motors_positions_worker_ = thread{[this]() {
    ...
    while (running_) {
      ...
      if (motors_positions_products_.insert(*generator)) {
        ...
        std::cerr << "motors_positions_worker_: ERROR consumer_producer "
                     "container is full"
                  << endl
                  << flush;
      }
      ...
      if (motors_positions.size() > 0 && robot_positions.size() > 0) {
        ...
        // Need to pass them to the active net function
        {
          ...
          if (match != net_functions_.end()) {
            ...
            l.unlock();
            // pass current logged values to active net function and receive new
            // calculated motors positions
            auto update_request = make_shared<motors_positions_t>(
            ...
            if (update_request->size() == config_.motors_count) {
              ...
              bool update_request_status =
                  motors_positions_update_request_.compare_exchange_strong(
                      expected, update_request);
              if (!update_request_status && running_) {
                cerr << "main_loop_t: Error - failed to set update_request "
                        "while engine is still running!"
                     << endl
                     << flush;
              }
            } else {
              std::cerr << "net function returned new motors_positions_t with "
                           "invalid size"
                        << endl
                        << flush;
            }
            // Currently allowing the external net_function to choose weather to
            // clear the values of the databases
          } else {
            std::cerr << "Info: net function not found" << endl << flush;
          }
        }
      } else {
        cerr << "main_loop Error: atleast one of the producer threads did not "
